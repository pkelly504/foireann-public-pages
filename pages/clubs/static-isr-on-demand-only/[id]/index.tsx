...
type StaticIsrTimedClubProps = {
  club?: Organization;
  places: Place[];
};

export default function StaticIsrTimedClub({ club, places }: StaticIsrTimedClubProps) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <h1>Welcome to {club.name} where everything is fetched at build time, but the page is only rebuilt when something on the org changes</h1>
...
export async function getStaticProps({
  ...
  params: { id: string };
}): Promise<GetStaticPropsResult<StaticIsrTimedClubProps>> {
  if (!process.env.JWT) {
    return {
      props: {
        club: undefined,
